...
const getKosDetails = async (kosId: number, getToken: () => string | null, hasValidToken: () => boolean) => {
  ...
  if (!response.ok) {
    throw new Error(result.error || 'Failed to fetch kos details');
  }
...
const updateKos = async (kosId: number, formData: EditKosFormData, getToken: () => string | null, hasValidToken: () => boolean) => {
  ...
  if (!response.ok) {
    throw new Error(result.error || 'Failed to update kos');
  }
...
export default function EditKosPage() {
  ...
  const [formData, setFormData] = useState<EditKosFormData>({
    title: '',
    description: '',
    price: 0,
    name: '',
      ...
      try {
        ...
        if (response.data) {
          ...
          setFormData({
            ...
            price: kosData.price || 0,
            name: kosData.name || '',
            address: kosData.address || '',
            city: kosData.city || '',
            facilities: kosData.facilities || '',
            totalRooms: kosData.totalRooms || 0,
            occupiedRooms: kosData.occupiedRooms || undefined,
            latitude: kosData.latitude || undefined,
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    setFormData(prev => ({
      ...prev,
      [name]: name === 'price' || name === 'totalRooms' || name === 'occupiedRooms' || name === 'latitude' || name === 'longitude' 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await showConfirm(
      'Apakah Anda yakin ingin menyimpan perubahan pada kos ini?',
    ...
    try {
      ...
      if (response.data) {
        ...
        await showSuccess(
          'Perubahan kos berhasil disimpan! Anda akan dialihkan ke halaman detail kos.',
          'Berhasil!'
        );
        ...
        <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="mb-8">
            ...
            <p className="text-gray-600 mt-2">
              Perbarui informasi kos Anda untuk menarik lebih banyak penyewa
            </p>
          </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder="500000"
                        min="0"
                        className={`text-gray-900 w-full pl-10 pr-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${
          ...
          <div className="mt-8 bg-amber-50 border border-amber-200 rounded-lg p-6">
            <h3 className="text-lg font-semibold text-amber-900 mb-4">⚠️ Tips Penting untuk Edit Kos</h3>
            <ul className="space-y-2 text-sm text-amber-800">
              <li className="flex items-start">
                <span className="inline-block w-2 h-2 bg-amber-400 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                <span>Pastikan semua informasi yang diubah akurat dan terbaru</span>
              </li>
              <li className="flex items-start">
                <span className="inline-block w-2 h-2 bg-amber-400 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                <span>Perubahan harga atau fasilitas dapat mempengaruhi minat penyewa</span>
              </li>
              <li className="flex items-start">
                <span className="inline-block w-2 h-2 bg-amber-400 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                <span>Update status kamar terisi secara berkala untuk transparansi</span>
              </li>
              <li className="flex items-start">
                <span className="inline-block w-2 h-2 bg-amber-400 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                <span>Jika mengubah lokasi atau alamat, pastikan koordinat juga diperbarui</span>
              </li>
              <li className="flex items-start">
                <span className="inline-block w-2 h-2 bg-amber-400 rounded-full mt-2 mr-3 flex-shrink-0"></span>
                <span>Perubahan akan langsung terlihat oleh calon penyewa setelah disimpan</span>
              </li>
            </ul>
          </div>
        </main>
